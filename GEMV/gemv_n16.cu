...
// A : M * N, N 是32的倍数，一个warp负责计算一行的结果
// V : N * 1
// 一行N个元素，则每一个warp一次性计算2行的16个元素的结果
...
__global__ void gemv_kernel(float* __restrict__ A, float* __restrict__ V,
  ...
  const int ITER_PER_ROW = MATRIX_N / N;
  int row_start = 2 * (bx * blockDim.y + ty);  // 计算当前所在行
  int tid = ty * blockDim.x + tx;              // 计算当前线程的块内id
  int flag = tid % WARP_SIZE;
  int row = 0;
  if (flag < N) {
    row = row_start;
  } else {
  ...
  for (int i = 0; i < ITER_PER_ROW; i++) {
    int col = i * N + landId;
    res += A[OFFSET(row, col, MATRIX_N)] * V[col];
  }
  // 然后通过warp reduce 进行求和
  unsigned int mask = 0xffffffff;
  res = warpReduceSum<N>(res, mask);
  if (landId == 0) {
    out[row] = res;
  }
}
int main() {
  ...
  const int WAPR_SIZE = 32;
  const int ROW_PER_WARP = 2;
  const int ELEMENT_PER_BLOCK = 128;
  const int WARP_PER_BLOCK = ELEMENT_PER_BLOCK / WAPR_SIZE;
  const int ROW_PER_BLOCK = ROW_PER_WARP * WARP_PER_BLOCK;
  // excute kernel
  for (int i = 0; i < nIter; i++) {
    dim3 threads_per_block = dim3(32, ELEMENT_PER_BLOCK / WAPR_SIZE);
    dim3 block_per_grid = MATRIX_M / ROW_PER_BLOCK;
